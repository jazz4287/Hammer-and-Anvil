...
class ModelReplacementStrategy(Strategy):
    def __init__(
            self,
        ...
        super(ModelReplacementStrategy, self).__init__(
            strategy_args, client_args, aggregator_args, backdoor_args, env_args
        )

    @staticmethod
    def sum(model_param_list):
        sum_model = {}
        for k in model_param_list[0].keys():
            sum_model[k] = torch.zeros_like(model_param_list[0][k], device=DEVICE)
            for model_param in model_param_list:
                sum_model[k] += model_param[k]
        return sum_model

    @staticmethod
    def compute_replacement_parameters(model_state_dict, benign_param_list, num_malicious):
        # we want modified update = (1/m)*(sum(benign)) - (n/m)*original_update
        n = len(benign_param_list) + num_malicious
        m = num_malicious
        s = ModelReplacementStrategy.sum(benign_param_list)
        new_model_state_dict = {}
        for k in model_state_dict.keys():
            new_model_state_dict[k] = torch.zeros_like(model_state_dict[k], device=DEVICE)
            new_model_state_dict[k] += ((n / m) * model_state_dict[k] - (1 / m) * s[k]).to(model_state_dict[k].dtype)
    ...
    def run(self):
        ...
        for epoch in range(self.strategy_args.num_epochs-1):  # we do everything as usual except for the last epoch
            ...
            for i in subset:
                ...
                if i < self.client_args.num_clients - self.client_args.num_malicious_clients:
                    ...
                    print("malicious")
                    client_model_state = client.train_one_epoch(aggregator_state_dict, self.benign_models, no_backdoor=True)  # we inject the backdoor at the end
        ...
        # we still have one more epoch to do, this is where the model replacement happens
        print(f"\nFinal epoch: Starting training.")
        aggregator_state_dict = aggregator.model.state_dict()
        ...
        self.benign_models = []
        self.malicious_models = []
        epoch = self.strategy_args.num_epochs-1
        # Step 2: inner loop looping over each client
        # TODO: parallize client creation/training for multi-gpu?
        subset = random.sample(range(self.client_args.num_clients), min(self.client_args.num_clients,
                                                                        max(1, self.client_args.subset_size)))
        subset.sort()
        for i in subset:
            print(f"Training client {i} for epoch {self.strategy_args.num_epochs-1}.")
            client = self.clients[i]
            client.set_epoch(epoch)
            client.set_id(i)
            # NOTE: client already has dataloader given from strategy.py
        ...
        for m in self.malicious_models:
            print(f"mal norm: {NormAggregator._get_norm(aggregator_state_dict, m)}")
        # we now aggregate the malicious models into a single malicious update
        mal_center = aggregator._avg_param(self.malicious_models, range(len(self.malicious_models)))

        new_mal_center = self.compute_replacement_parameters(mal_center, self.benign_models, len(self.malicious_models))

        self.malicious_models = [new_mal_center for _ in range(self.client_args.num_malicious_clients)]
        # Step 4: aggregate the parameters using the aggregator
        print(f"Epoch {epoch}: Training complete, aggregating models.")
        client_model_states = self.benign_models + self.malicious_models

        # Check that it works
        # mock_agg = aggregator.aggregate(client_model_states, mock=True).state_dict()
        # for k in mock_agg.keys():
        #     if mock_agg[k].dtype == torch.float:
        #         assert torch.allclose(mock_agg[k], mal_center[k])
